    ...
    def format_error(
        ...
        *,
        ...
        msg = "Found {} error{} in {} file{}".format(
            n_errors, "s" if n_errors != 1 else "", n_files, "s" if n_files != 1 else ""
        )
        if blockers:
            msg += " (errors prevented further checking)"
        else:
            msg += " (checked {} source file{})".format(
                n_sources, "s" if n_sources != 1 else ""
            )
        if not use_color:
            return msg
