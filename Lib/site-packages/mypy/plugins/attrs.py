...
attr_frozen_makers = {"attr.frozen"}  # type: Final
attr_define_makers = {"attr.define", "attr.mutable"}  # type: Final
attr_attrib_makers = {
    "attr.ib",
    "attr.attrib",
    "attr.attr",
...
class Converter:
    ...
    def __init__(
        self, name: Optional[str] = None, is_attr_converters_optional: bool = False
    ) -> None:
        self.name = name
...
class Attribute:
    ...
    def argument(self, ctx: "mypy.plugin.ClassDefContext") -> Argument:
        ...
        if self.converter.name:
            ...
            converter = lookup_qualified_stnode(
                ctx.api.modules, self.converter.name, True
            )
            if not converter:
                # The converter may be a local variable. Check there too.
                converter = ctx.api.lookup_qualified(
                    self.converter.name, self.info, True
                )
    ...
    def deserialize(
        ...
        return Attribute(
            ...
            data["kw_only"],
            Converter(
                data["converter_name"], data["converter_is_attr_converters_optional"]
            ),
        ...
        if attr_value:
            if isinstance(attr_value, NameExpr):
                ...
                if attr_value.fullname == "builtins.None":
                    return None
            ctx.api.fail(
                '"{}" argument must be True or False.'.format(name), ctx.reason
            )
            return default
        return default
    else:
...
def _attribute_from_auto_attrib(
    ...
    return Attribute(
        name, ctx.cls.info, has_rhs, True, kw_only, Converter(), stmt, init_type
    )
...
def _parse_converter(
    ctx: "mypy.plugin.ClassDefContext", converter: Optional[Expression]
) -> Converter:
