...
try:
    ...
    if sys.version_info >= (3, 8):
        ...
        assert "kind" in ast3.Constant._fields, (
            "This 3.8.0 alpha (%s) is too old; 3.8.0a3 required"
            % sys.version.split()[0]
        )
...
class ASTConverter:
    ...
    def set_line(
        self, node: N, n: Union[ast3.expr, ast3.stmt, ast3.ExceptHandler]
    ) -> N:
        node.line = n.lineno
        node.column = n.col_offset
        node.end_line = (
            getattr(n, "end_lineno", None) if isinstance(n, ast3.expr) else None
        )
        return node

    def translate_opt_expr_list(
        self, l: Sequence[Optional[AST]]
    ...
    def do_func_def(
        ...
        arg_names = [
            None if argument_elide_name(name) or name in posonlyargs else name
        ...
        func_def = FuncDef(
            n.name, args, self.as_required_block(n.body, lineno), func_type
        )
    ...
    def visit_AnnAssign(self, n: ast3.AnnAssign) -> AssignmentStmt:
        ...
        if n.value is None:  # always allow 'x: int'
            rvalue = TempNode(
                AnyType(TypeOfAny.special_form), no_rhs=True
            )  # type: Expression
            rvalue.line = line
    ...
    def visit_Lambda(self, n: ast3.Lambda) -> LambdaExpr:
        ...
        e = LambdaExpr(
            self.transform_args(n.args, n.lineno),
    ...
    def visit_FormattedValue(self, n: ast3.FormattedValue) -> Expression:
        ...
        conv_str = (
            "" if n.conversion is None or n.conversion < 0 else "!" + chr(n.conversion)
        )
...
class TypeConverter:
    ...
    def _extract_argument_name(self, n: ast3.expr) -> Optional[str]:
        if isinstance(n, Str):
            ...
            return None
        self.fail(
            "Expected string literal for argument name, got {}".format(
                type(n).__name__
            ),
            self.line,
            0,
    ...
    def visit_Subscript(self, n: ast3.Subscript) -> Type:
        if sys.version_info >= (3, 9):  # Really 3.9a5 or later
            ...
            if isinstance(sliceval, ast3.Slice) or (
                isinstance(sliceval, ast3.Tuple)
                and any(isinstance(x, ast3.Slice) for x in sliceval.elts)
            ):
                self.fail(
