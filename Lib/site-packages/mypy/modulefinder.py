...
class FindModuleCache:
    ...
    def get_toplevel_possibilities(
        ...
        if self.python_major_ver == 2:
            components = {
                id: filter_redundant_py2_dirs(dirs) for id, dirs in components.items()
            }
    ...
    ) -> Union[OnePackageDir, ModuleNotFoundReason]:
    ...
    def _can_find_module_in_parent_dir(self, id: str) -> bool:
        ...
        parent_search = FindModuleCache(
            SearchPaths((), (), (), ()), self.fscache, self.options
        )
        while any(
            file.endswith(("__init__.py", "__init__.pyi"))
...
def matches_exclude(
    subpath: str, exclude: str, fscache: FileSystemCache, verbose: bool
) -> bool:
    if not exclude:
        return False
...
def verify_module(fscache: FileSystemCache, id: str, path: str, prefix: str) -> bool:
    ...
    for i in range(id.count(".")):
        ...
        if not any(
            fscache.isfile_case(
                os.path.join(path, "__init__{}".format(extension)), prefix
            )
            for extension in PYTHON_EXTENSIONS
        ):
            return False
    ...
    if python_executable is None:
        ...
        site_packages = ast.literal_eval(
            subprocess.check_output(
                [python_executable, sitepkgs.__file__], stderr=subprocess.PIPE
            ).decode()
        )
...
def expand_site_packages(site_packages: List[str]) -> Tuple[List[str], List[str]]:
    ...
    for dir in site_packages:
        if not os.path.isdir(dir):
            continue
        pth_filenames = sorted(
            name for name in os.listdir(dir) if name.endswith(".pth")
        )
...
def compute_search_paths(
    ...
    for site_dir in site_packages:
        ...
        if (
            site_dir in mypypath
            or any(p.startswith(site_dir + os.path.sep) for p in mypypath)
            or os.path.altsep
            and any(p.startswith(site_dir + os.path.altsep) for p in mypypath)
        ):
            print(
                "{} is in the MYPYPATH. Please remove it.".format(site_dir),
                file=sys.stderr,
            )
            print(
            ...
            print(
                "{} is in the PYTHONPATH. Please change directory"
