...
class ConversionSpecifier:
    ...
    @classmethod
    def from_match(
        cls, match_obj: Match[str], non_standard_spec: bool = False
    ) -> "ConversionSpecifier":
...
class StringFormatterChecker:
    ...
    def parse_format_value(
        self, format_value: str, ctx: Context, nested: bool = False
        ...
        for target in top_targets:
            ...
            if match:
                ...
                if custom_match:
                    conv_spec = ConversionSpecifier.from_match(
            ...
            if (
                ...
                sub_conv_specs = self.parse_format_value(
                    conv_spec.format_spec, ctx=ctx, nested=True
                )
    ...
    def find_non_escaped_targets(
        self, format_value: str, ctx: Context
    ) -> Optional[List[str]]:
    ...
    def check_specs_in_format_call(
        ...
        for spec, repl in zip(specs, replacements):
            ...
            actual_type = (
                repl.type if isinstance(repl, TempNode) else self.chk.type_map.get(repl)
            )
            ...
            if not spec.type:
                ...
                expected_type = self.conversion_type(
                    spec.type, call, format_str, format_call=True
                )
    ...
    def perform_special_format_checks(
        ...
        if spec.flags:
            ...
            if (
                spec.type
                and spec.type not in NUMERIC_TYPES_NEW
                or not spec.type
    ...
    def auto_generate_keys(
        ...
        if some_defined and not all_defined:
            self.msg.fail(
                "Cannot combine automatic field numbering and"
    ...
    def apply_field_accessors(
        ...
        if temp_errors.is_errors():
            self.msg.fail(
                'Syntax error in format specifier "{}"'.format(spec.field),
                ctx,
                code=codes.STRING_FORMATTING,
            )
    ...
    def check_str_interpolation(
        ...
        if isinstance(expr, BytesExpr) and (3, 0) <= self.chk.options.python_version < (
            3,
            5,
        ):
            self.msg.fail(
    ...
    def check_mapping_str_interpolation(
        ...
        if isinstance(replacements, DictExpr) and all(
            isinstance(k, (StrExpr, BytesExpr, UnicodeExpr))
            for k, v in replacements.items
        ):
            mapping = {}  # type: Dict[str, Type]
            for k, v in replacements.items:
                if self.chk.options.python_version >= (3, 0) and isinstance(
                    expr, BytesExpr
                ):
                    # Special case: for bytes formatting keys must be bytes.
                    if not isinstance(k, BytesExpr):
                        self.msg.fail(
                            "Dictionary keys in bytes formatting must be bytes,"
                            " not strings",
                            expr,
                            code=codes.STRING_FORMATTING,
                        )
        ...
        if self.chk.options.python_version >= (3, 0):
            if isinstance(expr, BytesExpr):
                ...
                return self.chk.named_generic_type(
                    "typing.Mapping", [bytes_type, any_type]
                )
            elif isinstance(expr, StrExpr):
                str_type = self.chk.named_generic_type("builtins.str", [])
                return self.chk.named_generic_type(
                    "typing.Mapping", [str_type, any_type]
                )
            else:
            ...
            str_map = self.chk.named_generic_type(
                "typing.Mapping", [str_type, any_type]
            )
            unicode_map = self.chk.named_generic_type(
                "typing.Mapping", [unicode_type, any_type]
            )
    ...
    def checkers_for_regular_type(
        self, type: str, context: Context, expr: FormatStringExpr
    ) -> Optional[Checkers]:
        """Returns a tuple of check functions that check whether, respectively,
