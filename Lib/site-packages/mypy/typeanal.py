...
class TypeAnalyser(SyntheticTypeVisitor[Type], TypeAnalyzerPluginInterface):
            ...
            if isinstance(node, PlaceholderNode):
                if node.becomes_typeinfo:
                    ...
                    return PlaceholderType(
                        node.fullname, self.anal_array(t.args), t.line
                    )
                else:
                    if self.api.final_iteration:
                        self.cannot_resolve_type(t)
    ...
    def cannot_resolve_type(self, t: UnboundType) -> None:
        ...
        self.api.fail(
            'Cannot resolve name "{}" (possible cyclic definition)'.format(t.name), t
        )
    ...
    def visit_raw_expression_type(self, t: RawExpressionType) -> Type:
        ...
        if self.report_invalid_types:
            if t.base_type_name in ("builtins.int", "builtins.bool"):
                ...
                # a literal type.
                msg = "Invalid type: try using Literal[{}] instead?".format(
                    repr(t.literal_value)
                )
            elif t.base_type_name in ("builtins.float", "builtins.complex"):
                # We special-case warnings for floats and complex numbers.
                msg = "Invalid type: {} literals cannot be used as a type".format(
                    t.simple_name()
                )
            else:
    ...
    def analyze_callable_type(self, t: UnboundType) -> Type:
        ...
        if len(t.args) == 0:
            ...
            self.fail(
                'Please use "Callable[[<parameters>], <return type>]" or "Callable"', t
            )
            return AnyType(TypeOfAny.from_error)
    ...
    ) -> Optional[Tuple[List[Type], List[int], List[Optional[str]]]]:
        args = []  # type: List[Type]
        kinds = []  # type: List[int]
        ...
        for arg in arglist.items:
            if isinstance(arg, CallableArgument):
                ...
                if found is None:
                    ...
                    self.fail(
                        'Invalid argument constructor "{}"'.format(found.fullname), arg
                    )
                    return None
                else:
                    ...
                    if arg.name is not None and kind in {ARG_STAR, ARG_STAR2}:
                        self.fail(
                            "{} arguments should not have names".format(
                                arg.constructor
                            ),
                            arg,
                        )
    ...
    def analyze_literal_param(
        ...
        if isinstance(arg, UnboundType) and arg.original_str_expr is not None:
            ...
            return [
                LiteralType(
                    ...
                    fallback=self.named_type_with_normalized_str(
                        arg.original_str_fallback
                    ),
                    line=arg.line,
                    column=arg.column,
                )
        ...
        if isinstance(arg, AnyType):
            ...
            if arg.type_of_any not in (TypeOfAny.from_error, TypeOfAny.special_form):
                self.fail(
                    'Parameter {} of Literal[...] cannot be of type "Any"'.format(idx),
                    ctx,
                )
            return None
            ...
            if arg.literal_value is None:
                ...
                if name in ("float", "complex"):
                    msg = 'Parameter {} of Literal[...] cannot be of type "{}"'.format(
                        idx, name
                    )
                else:
    ...
    def infer_type_variables(
        ...
        for name, tvar_expr in type.ret_type.accept(
            TypeVarLikeQuery(
                self.lookup_qualified, self.tvar_scope, include_callables=False
            )
        ):
            if name not in names:
                names.append(name)
    ...
    def bind_function_type_variables(
        ...
        for name, tvar in typevars:
            if not self.tvar_scope.allow_binding(tvar.fullname):
                self.fail(
                    'Type variable "{}" is bound by an outer class'.format(name), defn
                )
...
MsgCallback = Callable[
    [str, Context, DefaultNamedArg(Optional[ErrorCode], "code")], None
]
...
def get_omitted_any(
    ...
    if disallow_any:
        ...
        if fullname in nongen_builtins:
            ...
            type_str = (
                typ.name if isinstance(typ, UnboundType) else format_type_bare(typ)
            )
...
def fix_instance(
    ...
    if act == "0":
        act = "none"
    fail(
        '"{}" expects {}, but {} given'.format(t.type.name, s, act),
        t,
        code=codes.TYPE_ARG,
    )
...
def set_any_tvars(
    ...
    *,
    ...
    if disallow_any:
        ...
        type_str = (
            otype.name if isinstance(otype, UnboundType) else format_type_bare(otype)
        )
