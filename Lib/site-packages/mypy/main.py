...
def main(
    ...
    if options.install_types and not sources:
        install_types(
            options.cache_dir, formatter, non_interactive=options.non_interactive
        )
        return
    ...
    if options.non_interactive:
        ...
        if missing_pkgs:
            ...
            install_types(
                options.cache_dir, formatter, after_run=True, non_interactive=True
            )
            fscache.flush()
            print()
    ...
    if options.install_types and not options.non_interactive:
        result = install_types(
            options.cache_dir, formatter, after_run=True, non_interactive=False
        )
        if result:
            print()
...
def process_options(
    ...
    add_invertible_flag(
        "--fast-exit", default=False, help=argparse.SUPPRESS, group=internals_group
    )
    ...
    if options.files and not (
        special_opts.files or special_opts.packages or special_opts.modules
    ):
    ...
    if invalid_codes:
        parser.error("Invalid error code(s): %s" % ", ".join(sorted(invalid_codes)))
    ...
    if special_opts.modules + special_opts.packages:
        ...
        for p in special_opts.packages:
            if os.sep in p or os.altsep and os.altsep in p:
                fail(
                    "Package name '{}' cannot have a slash in it.".format(p),
                    stderr,
                    options,
                )
    ...
    for root in options.package_root:
        ...
        if root:
            ...
            if root.startswith(dotdotslash):
                parser.error(
                    "Package root cannot be above current directory: %r" % root
                )
...
def process_cache_map(
    ...
    for i in range(0, n, 3):
        ...
        if not source.endswith(".py") and not source.endswith(".pyi"):
            parser.error(
                "Invalid --cache-map source %s (triple[0] must be *.py[i])" % source
            )
