...
def load_plugins_from_config(
    ...
    for plugin_path in options.plugins:
        ...
        try:
            ...
            print(
                "Error calling the plugin(version) entry point of {}\n".format(
                    plugin_path
                ),
                file=stdout,
            )
        ...
        try:
            ...
            print(
                "Error constructing plugin instance of {}\n".format(
...
class BuildManager:
    ...
    def __init__(
        ...
        self.find_module_cache = FindModuleCache(
            self.search_paths, self.fscache, self.options
        )
...
def write_deps_cache(
    ...
    for id in rdeps:
        ...
        if not manager.metastore.write(deps_json, deps_to_json(rdeps[id])):
            manager.log(
                "Error writing fine-grained deps JSON file {}".format(deps_json)
            )
            error = True
        else:
    ...
    if not metastore.write(DEPS_META_FILE, json.dumps(meta)):
        manager.log(
            "Error writing fine-grained deps meta JSON file {}".format(DEPS_META_FILE)
        )
        error = True
...
def write_plugins_snapshot(manager: BuildManager) -> None:
    ...
    if not manager.metastore.write(
        PLUGIN_SNAPSHOT_FILE, json.dumps(manager.plugins_snapshot)
    ):
        manager.errors.set_file(_cache_dir_prefix(manager.options), None)
...
def read_plugins_snapshot(manager: BuildManager) -> Optional[Dict[str, str]]:
    ...
    if not isinstance(snapshot, dict):
        manager.log(
            "Could not load plugins snapshot: cache is not a dict: {}".format(
                type(snapshot)
            )
        )
        return None
...
def read_quickstart_file(
    ...
    if options.quickstart_file:
        ...
        try:
            ...
            print(
                "Warning: Failed to load quickstart file: {}\n".format(str(e)),
                file=stdout,
            )
    return quickstart
...
def read_deps_cache(
    ...
    if not manager.options.skip_cache_mtime_checks:
        for id, meta in module_deps_metas.items():
            ...
            if not matched:
                manager.log(
                    "Invalid or missing fine-grained deps cache: {}".format(
                        meta["path"]
                    )
                )
                return None
...
def find_cache_meta(id: str, path: str, manager: BuildManager) -> Optional[CacheMeta]:
    ...
    if not isinstance(meta, dict):
        manager.log(
            "Could not load cache for {}: meta cache is not a dict: {}".format(
                id, repr(meta)
            )
        )
        return None
    ...
    if m.plugin_data != plugin_data:
        manager.log(
            "Metadata abandoned for {}: plugin configuration differs".format(id)
        )
        return None
...
def validate_meta(
    ...
    if meta.ignore_all and not ignore_all:
        manager.log(
            "Metadata abandoned for {}: errors were previously ignored".format(id)
        )
        return None
    ...
    if not stat.S_ISREG(st.st_mode):
        manager.log(
            "Metadata abandoned for {}: file {} does not exist".format(id, path)
        )
        return None
    ...
    if size != meta.size and not bazel and not fine_grained_cache:
        manager.log(
            "Metadata abandoned for {}: file {} has different size".format(id, path)
        )
        return None
    ...
    if not bazel and (mtime != meta.mtime or path != meta.path):
        if manager.quickstart_state and path in manager.quickstart_state:
            ...
            if int(qmtime) == mtime and qsize == size and qhash == meta.hash:
                manager.log(
                    "Metadata fresh (by quickstart) for {}: file {}".format(id, path)
                )
        ...
        if source_hash != meta.hash:
            if fine_grained_cache:
                ...
                manager.log(
                    "Metadata abandoned for {}: file {} has different hash".format(
                        id, path
                    )
                )
                return None
...
def delete_cache(id: str, path: str, manager: BuildManager) -> None:
    ...
    manager.log(
        "Deleting {} {} {}".format(id, path, " ".join(x for x in cache_paths if x))
    )
    ...
    for filename in cache_paths:
        try:
            ...
            if e.errno != errno.ENOENT:
                manager.log(
                    "Error deleting cache file {}: {}".format(filename, e.strerror)
                )
...
class State:
    ...
    source_hash = (
        None
    )  # type: Optional[str]  # Hash calculated based on the source code
    meta_source_hash = (
        None
    ...
    dependencies_set = (
        None
    )  # type: Set[str]  # The same but as a set for deduplication purposes
    ...
    def __init__(
        ...
        if self.meta:
            ...
            if manager.use_fine_grained_cache():
                manager.log(
                    "Deferring module to fine-grained update %s (%s)" % (path, id)
                )
            ...
            if self.path and source is None:
                try:
                    ...
                    if self.path.endswith(".pyd"):
                        err = "mypy: stubgen does not support .pyd files: '{}'".format(
                            self.path
                        )
                    else:
                        err = "mypy: can't decode file '{}': {}".format(
    ...
    def finish_passes(self) -> None:
        ...
        with self.wrap_context():
            ...
            self._patch_indirect_dependencies(
                self.type_checker().module_refs, self.type_map()
            )
    ...
    def write_cache(self) -> None:
        ...
        assert len(set(self.dependencies)) == len(
            self.dependencies
        ), "Duplicates in dependencies list for {} ({})".format(
            ...
            try:
                if dep in self.ancestors:
                    state, ancestor = (
                        None,
                        self,
                    )  # type: (Optional[State], Optional[State])
                else:
...
def find_module_and_diagnose(
    ...
    if isinstance(result, str):
        ...
        if not manager.options.no_silence_site_packages:
            for dir in (
                manager.search_paths.package_path + manager.search_paths.typeshed_path
            ):
        ...
        if (
            ...
            raise CompileError(
                [
                    'mypy: "%s" shadows library module "%s"'
                    % (os.path.relpath(result), id),
                    'note: A user-defined top-level module with name "%s" is not supported'
                    % id,
                ]
            )
...
def module_not_found(
    ...
    if target == "builtins":
        ...
        errors.report(
            line, 0, msg.format(module=target, pyver=pyver), code=codes.IMPORT
        )
        ...
        for note in notes:
            ...
            errors.report(
                line, 0, note, severity="note", only_once=True, code=codes.IMPORT
            )
...
def skipping_module(
    manager: BuildManager, line: int, caller_state: Optional[State], id: str, path: str
) -> None:
    ...
    manager.errors.report(line, 0, 'Import of "%s" ignored' % (id,), severity="error")
    manager.errors.report(
        line,
        0,
    ...
    for bs in sources:
        ...
        if st.id in graph:
            ...
            manager.errors.report(
                -1,
                -1,
                'Duplicate module named "%s" (also at "%s")'
                % (st.id, graph[st.id].xpath),
                blocker=True,
            )
            manager.errors.report(
                -1,
    ...
    for ascc in sccs:
        ...
        if fresh:
            ...
            if len(fresh_scc_queue) > 0:
                manager.log(
                    "Processing {} queued fresh SCCs".format(len(fresh_scc_queue))
                )
