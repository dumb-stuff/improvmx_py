...
class SemanticAnalyzer(
    ...
    plugin = (
        None
    )  # type: Plugin     # Mypy plugin for special casing of library features
        ...
        for alias, target_name in type_aliases.items():
            ...
            if n:
                if isinstance(n.node, PlaceholderNode):
                    ...
                    fix_instance_types(
                        target, self.fail, self.note, self.options.python_version
                    )
    ...
    def set_original_def(
        self, previous: Optional[Node], new: Union[FuncDef, Decorator]
    ) -> bool:
    ...
    def analyze_property_with_multi_part_definition(
        ...
        for i, item in enumerate(items[1:]):
            if isinstance(item, Decorator):
                ...
                self.fail(
                    'Unexpected definition for property "{}"'.format(
    ...
    def add_function_to_symbol_table(
        self, func: Union[FuncDef, OverloadedFuncDef]
    ) -> None:
        if self.is_class_scope():
            assert self.type is not None
            func.info = self.type
    ...
    def visit_decorator(self, dec: Decorator) -> None:
        ...
        for i, d in enumerate(dec.decorators):
            ...
            if refers_to_fullname(d, "abc.abstractmethod"):
                ...
                refers_to_fullname(d, "builtins.property")
                or refers_to_fullname(d, "abc.abstractproperty")
                or refers_to_fullname(d, "functools.cached_property")
            ):
                removed.append(i)
                ...
                no_type_check = True
            elif refers_to_fullname(d, "typing.final") or refers_to_fullname(
                d, "typing_extensions.final"
            ):
                if self.is_class_scope():
    ...
    def analyze_class_decorator(self, defn: ClassDef, decorator: Expression) -> None:
        ...
        if isinstance(decorator, RefExpr):
            ...
            elif decorator.fullname in ("typing.final", "typing_extensions.final"):
    ...
    ) -> Tuple[List[Expression], List[TypeVarDef], bool]:
    ...
    def analyze_class_typevar_declaration(
        ...
        if (
            ...
            or sym.node.fullname == "typing.Protocol"
            and base.args
            or sym.node.fullname == "typing_extensions.Protocol"
            and base.args
        ):
    ...
    def prepare_class_def(
        self, defn: ClassDef, info: Optional[TypeInfo] = None
    ) -> None:
    ...
    def analyze_base_classes(
        ...
        for base_expr in base_type_exprs:
            if (
                ...
                and base_expr.fullname in ("typing.NamedTuple",) + TPDICT_NAMES
            ):
        ...
        * past.utils.with_metaclass(M, B1, B2, ...)
        ...
        if self.options.python_version[0] == 2:
            for body_node in defn.defs.body:
                if (
                    isinstance(body_node, ClassDef)
                    and body_node.name == "__metaclass__"
                ):
                    self.fail(
                    ...
                    isinstance(body_node, AssignmentStmt)
                    and len(body_node.lvalues) == 1
                ):
        ...
        metas = {defn.metaclass, python2_meta_expr, with_meta_expr, add_meta_expr} - {
            None
        }
        if len(metas) == 0:
            return
    ...
    def verify_base_classes(self, defn: ClassDef) -> bool:
        ...
        for base in info.bases:
            ...
            if baseinfo.fullname == "builtins.bool":
                self.fail(
                    '"%s" is not a valid base class' % baseinfo.name, defn, blocker=True
                )
                return False
    ...
    def analyze_lvalues(self, s: AssignmentStmt) -> None:
        ...
        for lval in s.lvalues:
            self.analyze_lvalue(lval, explicit_type=explicit, is_final=s.is_final_def)
    ...
    def analyze_lvalue(
        ...
        if isinstance(lval, NameExpr):
            self.analyze_name_lvalue(
                lval, explicit_type, is_final, escape_comprehensions
            )
    ...
    def analyze_member_lvalue(
        self, lval: MemberExpr, explicit_type: bool, is_final: bool
    ) -> None:
    ...
    def process_typevar_declaration(self, s: AssignmentStmt) -> bool:
        ...
        if not call.analyzed:
            type_var = TypeVarExpr(
                name, self.qualified_name(name), values, upper_bound, variance
            )
            ...
            if (
                ...
                or call.analyzed.upper_bound != upper_bound
            ):
                self.progress = True
    ...
    def check_typevarlike_name(
        ...
        if (
            ...
            self.fail(
                "{}() expects a string literal as first argument".format(
                    typevarlike_type
                ),
                context,
            )
        ...
        for node in items:
            try:
                analyzed = self.anal_type(
                    expr_to_unanalyzed_type(node), allow_placeholder=True
                )
                if analyzed is None:
                    # Type variables are special: we need to place them in the symbol table
    ...
    def visit_yield_from_expr(self, e: YieldFromExpr) -> None:
        if not self.is_func_scope():  # not sure
            ...
            if self.function_stack[-1].is_coroutine:
                self.fail(
                    '"yield from" in async function', e, serious=True, blocker=True
                )
            else:
    ...
    def analyze_type_application_args(self, expr: IndexExpr) -> Optional[List[Type]]:
        ...
        for item in items:
            ...
            analyzed = self.anal_type(
                typearg, allow_unbound_tvars=True, allow_placeholder=True
            )
            if analyzed is None:
                return None
        ...
        if not self.is_func_scope():
            ...
            if self.function_stack[-1].is_coroutine:
                if self.options.python_version < (3, 6):
                    self.fail(
                        '"yield" in async function', expr, serious=True, blocker=True
                    )
                else:
                    self.function_stack[-1].is_generator = True
    ...
    def add_local(
        self, node: Union[Var, FuncDef, OverloadedFuncDef], context: Context
    ) -> None:
    ...
    def cannot_resolve_name(self, name: str, kind: str, ctx: Context) -> None:
        self.fail(
            'Cannot resolve {} "{}" (possible cyclic definition)'.format(kind, name),
            ctx,
        )
    ...
    def check_no_global(
        self, name: str, ctx: Context, is_overloaded_func: bool = False
    ) -> None:
        if name in self.globals:
    ...
    def name_not_defined(
        self, name: str, ctx: Context, namespace: Optional[str] = None
    ) -> None:
    ...
    def already_defined(
        ...
        self.fail(
            '{} "{}" already defined{}'.format(noun, unmangle(name), extra_msg),
            ctx,
            code=codes.NO_REDEF,
        )
    ...
    def fail(
        ...
        *,
        ...
        self.errors.report(
            ctx.get_line(), ctx.get_column(), msg, blocker=blocker, code=code
        )
    ...
    def note(self, msg: str, ctx: Context, code: Optional[ErrorCode] = None) -> None:
        if (
            ...
            return
        self.errors.report(
            ctx.get_line(), ctx.get_column(), msg, severity="note", code=code
        )
...
def is_same_var_from_getattr(
    n1: Optional[SymbolNode], n2: Optional[SymbolNode]
) -> bool:
