...
class MessageBuilder:
    ...
    def note(
        ...
        *,
        ...
        self.report(
            msg, context, "note", file=file, origin=origin, offset=offset, code=code
        )
    ...
    def has_no_attr(
        ...
        if isinstance(
            ...
            self.fail(
                "Unsupported right operand type for in ({})".format(
            ...
            self.fail(
                "Unsupported operand type for unary - ({})".format(
                    format_type(original_type)
                ),
                context,
                code=codes.OPERATOR,
            ...
            if isinstance(original_type, CallableType) and original_type.is_type_obj():
                ...
                self.fail(
                    "Value of type {} is not indexable".format(
                        format_type(original_type)
                    ),
                    context,
                    code=codes.INDEX,
            ...
            if isinstance(original_type, Instance) and (
                ...
                self.fail(
                    "{} not callable".format(format_type(original_type)),
                    context,
                    code=codes.OPERATOR,
                )
        else:
    ...
    def untyped_function_call(self, callee: CallableType, context: Context) -> Type:
        ...
        self.fail(
            "Call to untyped function {} in typed context".format(name),
            context,
    ...
    def too_many_arguments_from_typed_dict(
        ...
        for key in arg_type.items:
            if key not in callee.arg_names:
                msg = 'Extra argument "{}" from **args'.format(key) + for_function(
                    callee
                )
                break
        else:
    ...
    def does_not_return_value(
        self, callee_type: Optional[Type], context: Context
    ) -> None:
        ...
        if name is not None:
            self.fail(
                "{} does not return a value".format(capitalize(name)),
                context,
    ...
    def no_variant_matches_arguments(
        ...
        *,
        ...
        if num_args == 0:
            self.fail(
                "All overload variants{} require at least one argument".format(
                    name_str
                ),
                context,
                code=code,
            ...
            self.fail(
                "No overload variant{} matches argument type {}".format(
                    name_str, arg_types_str
                ),
                context,
                code=code,
            )
            ...
            self.fail(
                "No overload variant{} matches argument types {}".format(
                    name_str, arg_types_str
                ),
                context,
                code=code,
        ...
        self.pretty_overload_matches(
            plausible_targets, overload, context, offset=2, max_items=2, code=code
        )
    ...
    def could_not_infer_type_arguments(
        ...
        if callee_name is not None and n > 0:
            self.fail(
                "Cannot infer type argument {} of {}".format(n, callee_name), context
            )
        else:
    ...
    def first_argument_for_super_must_be_type(
        ...
        self.fail(
            'Argument 1 for "super" must be a type object; got {}'.format(type_str),
            context,
            code=codes.ARG_TYPE,
        )
    ...
    def unsupported_placeholder(self, placeholder: str, context: Context) -> None:
        self.fail(
            'Unsupported format character "%s"' % placeholder,
            context,
            code=codes.STRING_FORMATTING,
        )
    ...
    def key_not_in_mapping(self, key: str, context: Context) -> None:
        self.fail(
            'Key "%s" not found in mapping' % key, context, code=codes.STRING_FORMATTING
        )
    ...
    def cant_assign_to_method(self, context: Context) -> None:
        self.fail(
            message_registry.CANNOT_ASSIGN_TO_METHOD, context, code=codes.ASSIGNMENT
        )
    ...
    def final_cant_override_writable(self, name: str, ctx: Context) -> None:
        self.fail(
            'Cannot override writable attribute "{}" with a final one'.format(name), ctx
        )
    ...
    def overloaded_signatures_typevar_specific(
        ...
        self.fail(
            "Overloaded function implementation cannot satisfy signature {} ".format(
                index
            )
    ...
    def warn_operand_was_from_union(
        ...
        self.note(
            "{} operand is of type {}".format(side, format_type(original)),
            context,
            code=codes.OPERATOR,
        )
    ...
    def invalid_signature_for_special_method(
        ...
        self.fail(
            'Invalid signature "{}" for "{}"'.format(func_type, method_name), context
        )
    ...
    def reveal_locals(
        self, type_map: Dict[str, Optional[Type]], context: Context
    ) -> None:
    ...
    def unsupported_type_type(self, item: Type, context: Context) -> None:
        self.fail(
            'Cannot instantiate type "Type[{}]"'.format(format_type_bare(item)), context
        )

    def redundant_cast(self, typ: Type, context: Context) -> None:
        self.fail(
            "Redundant cast to {}".format(format_type(typ)),
            context,
    ...
    def unimported_type_becomes_any(self, prefix: str, typ: Type, ctx: Context) -> None:
        self.fail(
            "{} becomes {} due to an unfollowed import".format(
                prefix, format_type(typ)
            ),
            ctx,
    ...
    def need_annotation_for_var(
        ...
        if (
            ...
            if has_variable_annotations:
                ...
                hint = ' (hint: "{} = ...  # type: {}[{}]")'.format(
                    node.name, alias, type_dec
                )
        ...
        self.fail(
            'Need type {} for "{}"{}'.format(needed, unmangle(node.name), hint),
            context,
    ...
    def unexpected_typeddict_keys(
        ...
        self.fail(
            "Expected {} but found {}".format(expected, found),
            context,
            code=codes.TYPEDDICT_ITEM,
        )
    ...
    def typeddict_key_not_found(
        ...
        if typ.is_anonymous():
            ...
            if matches:
                self.note(
                    "Did you mean {}?".format(pretty_seq(matches[:3], "or")), context
                )
    ...
    def typeddict_context_ambiguous(
        ...
        self.fail(
            "Type of TypedDict is ambiguous, could be any of ({})".format(
                formatted_types
            ),
            context,
        )
    ...
    def typeddict_key_cannot_be_deleted(
        ...
        if typ.is_anonymous():
            self.fail('TypedDict key "{}" cannot be deleted'.format(item_name), context)
        else:
            self.fail(
                'Key "{}" of TypedDict {} cannot be deleted'.format(
                    item_name, format_type(typ)
                ),
                context,
            )
    ...
    def disallowed_any_type(self, typ: Type, context: Context) -> None:
        ...
        if isinstance(typ, AnyType):
            ...
            message = 'Expression type contains "Any" (has type {})'.format(
                format_type(typ)
            )
    ...
    def untyped_decorated_function(self, typ: Type, context: Context) -> None:
        ...
        if isinstance(typ, AnyType):
            ...
            self.fail(
                'Type of decorated function contains type "Any" ({})'.format(
                    format_type(typ)
                ),
                context,
            )
    ...
    def typed_function_untyped_decorator(
        ...
        self.fail(
            'Untyped decorator makes function "{}" untyped'.format(func_name), context
        )
    ...
    def concrete_only_assign(self, typ: Type, context: Context) -> None:
        self.fail(
            "Can only assign concrete classes to a variable of type {}".format(
                format_type(typ)
            ),
            context,
        )
    ...
    def concrete_only_call(self, typ: Type, context: Context) -> None:
        self.fail(
            "Only concrete class can be given where {} is expected".format(
                format_type(typ)
            ),
            context,
        )
    ...
    def cannot_use_function_with_type(
        ...
        self.fail(
            "Cannot use {}() with {} type".format(method_name, type_name), context
        )
    ...
    def report_non_method_protocol(
        ...
        self.fail(
            "Only protocols that don't have non-method members can be"
            " used with issubclass()",
            context,
        )
        if len(members) < 3:
            attrs = ", ".join(members)
            self.note(
                'Protocol "{}" has non-method member(s): {}'.format(tp.name, attrs),
                context,
            )
    ...
    def redundant_left_operand(self, op_name: str, context: Context) -> None:
        ...
        """
        self.redundant_expr(
            'Left operand of "{}"'.format(op_name), op_name == "and", context
        )
    ...
    def redundant_expr(
        ...
        self.fail(
            "{} is always {}".format(description, str(truthiness).lower()),
            context,
        ...
        if conflict_types and (
            ...
            for name, got, exp in conflict_types[:MAX_ITEMS]:
                ...
                if not isinstance(exp, (CallableType, Overloaded)) or not isinstance(
                    ...
                    self.note(
                        "{}: expected {}, got {}".format(
                            name, *format_type_distinctly(exp, got)
                        ),
                        context,
                        offset=OFFSET,
                        code=code,
                    ...
                    if isinstance(exp, CallableType):
                        self.note(
                            pretty_callable(exp), context, offset=2 * OFFSET, code=code
                        )
                    else:
                    ...
                    if isinstance(got, CallableType):
                        self.note(
                            pretty_callable(got), context, offset=2 * OFFSET, code=code
                        )
                    else:
    ...
    def pretty_overload_matches(
        ...
        self.note(
            "Possible overload variant{}:".format(plural_s(shown)), context, code=code
        )
        ...
        if shown < max_matching <= max_available:
            ...
            msg = "<{} more non-matching overload{} not shown>".format(
                left, plural_s(left)
            )
            self.note(msg, context, offset=2 * offset, code=code)
        else:
    ...
    def print_more(
        ...
        *,
        ...
        if len(conflicts) > max_items:
            self.note(
                "<{} more conflict(s) not shown>".format(len(conflicts) - max_items),
                context,
                offset=offset,
                code=code,
            )
    ...
    def format_long_tuple_type(self, typ: TupleType) -> str:
        ...
        if item_cnt > 10:
            return "Tuple[{}, {}, ... <{} more items>]".format(
                format_type_bare(typ.items[0]),
                format_type_bare(typ.items[1]),
                str(item_cnt - 2),
            )
        else:
                ...
                if error_cnt < 3:
                    notes.append(
                        'Expression tuple item {} has type "{}"; "{}" expected; '.format(
                            str(i), format_type_bare(rhs_t), format_type_bare(lhs_t)
                        )
                    )
                error_cnt += 1
...
def format_type_inner(typ: Type, verbosity: int, fullnames: Optional[Set[str]]) -> str:
    ...
    if isinstance(typ, Instance):
        ...
        for (item_name, item_type) in typ.items.items():
            ...
            items.append("{!r}{}: {}".format(item_name, modifier, format(item_type)))
        ...
        if func.is_type_obj():
            ...
            return format(
                TypeType.make_normalized(erase_type(func.items()[0].ret_type))
            )
            ...
            for arg_name, arg_type, arg_kind in zip(
                ...
                if (
                    ...
                    or verbosity == 0
                    and arg_kind in (ARG_POS, ARG_OPT)
                ):
                    ...
                    if arg_kind in (ARG_STAR, ARG_STAR2) or arg_name is None:
                        arg_strings.append(
                            "{}({})".format(constructor, format(arg_type))
                        )
                    else:
                        arg_strings.append(
                            "{}({}, {})".format(
                                constructor, format(arg_type), repr(arg_name)
                            )
                        )
...
def format_type_bare(
    typ: Type, verbosity: int = 0, fullnames: Optional[Set[str]] = None
) -> str:
    """
    Convert a type to a relatively short string suitable for error messages.
        ...
        for tvar in tp.variables:
            if isinstance(tvar, TypeVarDef):
                ...
                if (
                    ...
                    tvars.append(
                        "{} <: {}".format(tvar.name, format_type_bare(upper_bound))
                    )
                elif tvar.values:
                    tvars.append(
                        "{} in ({})".format(
                            tvar.name,
                            ", ".join([format_type_bare(tp) for tp in tvar.values]),
                        )
                    )
                else:
