...
class StubSource:
    ...
    def __init__(
        ...
        runtime_all: Optional[List[str]] = None,
    ) -> None:
...
class ImportTracker:
    ...
    def add_import_from(
        self, module: str, names: List[Tuple[str, Optional[str]]]
    ) -> None:
...
class StubGenerator(mypy.traverser.TraverserVisitor):
    ...
    def visit_func_def(
        ...
        self.clear_decorators()
        self.add(
            "%s%sdef %s(" % (self._indent, "async " if o.is_coroutine else "", o.name)
        )
        ...
        for i, arg_ in enumerate(o.arguments):
            ...
            if arg_.initializer:
                if kind in (ARG_NAMED, ARG_NAMED_OPT) and not any(
    ...
    def process_decorator(self, o: Decorator) -> Tuple[bool, bool]:
        ...
        for decorator in o.original_decorators:
            if isinstance(decorator, NameExpr):
                i_is_abstract, i_is_overload = self.process_name_expr_decorator(
                    decorator, o
                )
                is_abstract = is_abstract or i_is_abstract
                is_overload = is_overload or i_is_overload
            elif isinstance(decorator, MemberExpr):
                i_is_abstract, i_is_overload = self.process_member_expr_decorator(
                    decorator, o
                )
                is_abstract = is_abstract or i_is_abstract
                is_overload = is_overload or i_is_overload
    ...
    def process_name_expr_decorator(
        self, expr: NameExpr, context: Decorator
    ) -> Tuple[bool, bool]:
        ...
        if expr.name == "setter" and isinstance(expr.expr, NameExpr):
            ...
            if (
                ...
                and (
                    ...
                    or self.import_tracker.reverse_alias.get(expr.expr.expr.name)
                    == "asyncio"
                )
            ):
            ...
            isinstance(expr.expr, NameExpr)
            and (
                expr.expr.name == "typing"
                or self.import_tracker.reverse_alias.get(expr.expr.name) == "typing"
            )
        ...
        for lvalue in o.lvalues:
            ...
            if isinstance(lvalue, TupleExpr) or isinstance(lvalue, ListExpr):
                ...
                if isinstance(o.unanalyzed_type, TupleType):  # type: ignore
                    annotations = (
                        o.unanalyzed_type.items
                    )  # type: Iterable[Optional[Type]]
                else:
    ...
    def visit_import_from(self, o: ImportFrom) -> None:
        ...
        for name, as_name in o.names:
            ...
            if (
                as_name is None
                and self.module
                and (self.module + "." + name) in EXTRA_EXPORTED
            ):
            ...
            if (
                ...
                and not is_private
                and top_level
                in (self.module.split(".")[0], "_" + self.module.split(".")[0])
            ):
    ...
    def add_coroutine_decorator(
        self, func: FuncDef, name: str, require_name: str
    ) -> None:
    ...
    def is_private_name(self, name: str, fullname: Optional[str] = None) -> bool:
        ...
        return name.startswith("_") and (
            not name.endswith("__") or name in IGNORED_DUNDERS
        )
...
def generate_stubs(options: Options) -> None:
    ...
    for mod in c_modules:
        if any(
            py_mod.module.startswith(mod.module + ".")
        ...
        with generate_guarded(
            ...
            generate_stub_for_c_module(
                mod.module, target, sigs=sigs, class_sigs=class_sigs
            )
...
def parse_options(args: List[str]) -> Options:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "don't implicitly export all names imported from other modules "
            "in the same package"
        ),
    )
    parser.add_argument(
    ...
    if not ns.interpreter:
        ns.interpreter = (
            sys.executable if pyversion[0] == 3 else default_py2_interpreter()
        )
