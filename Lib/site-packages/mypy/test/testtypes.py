...
class TypesSuite(Suite):
    ...
    def test_tuple_type(self) -> None:
        ...
        assert_equal(
            str(
                TupleType([self.x, AnyType(TypeOfAny.special_form)], self.fx.std_tuple)
            ),
            "Tuple[X?, Any]",
        )
    ...
    def test_type_variable_binding(self) -> None:
        ...
        assert_equal(
            str(TypeVarDef("X", "X", 1, [self.x, self.y], self.fx.o)), "X in (X?, Y?)"
        )
...
class TypeOpsSuite(Suite):
    ...
    def test_is_more_precise(self) -> None:
        ...
        assert_true(is_more_precise(self.tuple(fx.b, fx.a), self.tuple(fx.b, fx.a)))
    ...
    def test_is_proper_subtype(self) -> None:
        ...
        assert_true(
            is_proper_subtype(UnionType([fx.a, fx.b]), UnionType([fx.a, fx.b, fx.c]))
        )
        assert_false(
            is_proper_subtype(UnionType([fx.a, fx.b]), UnionType([fx.b, fx.c]))
        )
    ...
    def test_simplified_union(self) -> None:
        ...
        self.assert_simplified_union(
            [fx.lit1, fx.lit1_inst], UnionType([fx.lit1, fx.lit1_inst])
        )
        self.assert_simplified_union(
...
class JoinSuite(Suite):
    ...
    def assert_simple_join(self, s: Type, t: Type, join: Type) -> None:
        ...
        expected = str(join)
        assert_equal(
            actual, expected, "join({}, {}) == {{}} ({{}} expected)".format(s, t)
        )
...
class MeetSuite(Suite):
    ...
    def assert_simple_meet(self, s: Type, t: Type, meet: Type) -> None:
        ...
        expected = str(meet)
        assert_equal(
            actual, expected, "meet({}, {}) == {{}} ({{}} expected)".format(s, t)
        )
...
class SameTypeSuite(Suite):
    ...
    def assert_simple_is_same(
        ...
        if strict:
            actual2 = s == t
            assert_equal(
                actual2, expected, "({} == {}) is {{}} ({{}} expected)".format(s, t)
            )
            assert_equal(
                hash(s) == hash(t),
                expected,
