...
class ASTMergeSuite(DataSuite):
    ...
    def dump_symbol_table(self, module_id: str, symtable: SymbolTable) -> List[str]:
        ...
        for name in sorted(symtable):
            if name.startswith("__"):
                continue
            a.append(
                "    {}: {}".format(name, self.format_symbol_table_node(symtable[name]))
            )
        return a
    ...
    def format_symbol_table_node(self, node: SymbolTableNode) -> str:
        ...
        if isinstance(node.node, Node):
            s = "{}<{}>".format(
                str(type(node.node).__name__), self.id_mapper.id(node.node)
            )
        else:
    ...
    def dump_types(self, manager: FineGrainedBuildManager) -> List[str]:
        ...
        for module_id in sorted(manager.manager.modules):
            ...
            if type_map:
                ...
                for expr in sorted(
                    type_map,
                    key=lambda n: (n.line, short_type(n), str(n) + str(type_map[n])),
                ):
                    typ = type_map[expr]
                    a.append(
                        "{}:{}: {}".format(
                            short_type(expr), expr.line, self.format_type(typ)
                        )
                    )
        return a
