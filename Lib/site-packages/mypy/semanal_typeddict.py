...
class TypedDictAnalyzer:
    ...
    def analyze_typeddict_classdef(
        ...
        if possible:
            ...
            if any(
                not isinstance(expr, RefExpr)
                or expr.fullname not in TPDICT_NAMES
                and not self.is_typeddict(expr)
            ...
            for base in reversed(typeddict_bases):
                ...
                for key in base_items:
                    if key in keys:
                        self.fail(
                            'Overwriting TypedDict field "{}" while merging'.format(
                                key
                            ),
                            defn,
                        )
            ...
            info = self.build_typeddict_typeinfo(
                defn.name, keys, types, required_keys, defn.line
            )
            defn.analyzed = TypedDictExpr(info)
            defn.analyzed.line = defn.line
            defn.analyzed.column = defn.column
        ...
        for stmt in defn.defs.body:
            if not isinstance(stmt, AssignmentStmt):
                ...
                if name in (oldfields or []):
                    self.fail(
                        'Overwriting TypedDict field "{}" while extending'.format(name),
                        stmt,
                    )
                if name in fields:
    ...
    def check_typeddict(
        ...
        if not ok:
            ...
            info = self.build_typeddict_typeinfo(
                name, items, types, required_keys, call.line
            )
            info.line = node.line
    ...
    def parse_typeddict_fields_with_types(
        ...
        for (field_name_expr, field_type_expr) in dict_items:
            if isinstance(field_name_expr, (StrExpr, BytesExpr, UnicodeExpr)):
                ...
                if key in seen_keys:
                    self.fail(
                        'Duplicate TypedDict key "{}"'.format(key), field_name_expr
                    )
                seen_keys.add(key)
            else:
