...
class NewTypeAnalyzer:
    ...
    def process_newtype_declaration(self, s: AssignmentStmt) -> bool:
        ...
        if isinstance(old_type, TupleType):
            ...
            newtype_class_info = self.build_newtype_typeinfo(
                name, old_type, old_type, s.line
            )
        else:
            if old_type is not None:
                message = "Argument 2 to NewType(...) must be subclassable (got {})"
                self.fail(
                    message.format(format_type(old_type)), s, code=codes.VALID_NEWTYPE
                )
            ...
            newtype_class_info = self.build_newtype_typeinfo(
                name, old_type, object_type, s.line
            )
    ...
    def check_newtype_args(
        ...
        old_type = get_proper_type(
            self.api.anal_type(unanalyzed_type, report_invalid_types=False)
        )
        should_defer = False
