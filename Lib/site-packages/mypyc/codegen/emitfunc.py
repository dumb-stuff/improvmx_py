...
def native_function_header(fn: FuncDecl, emitter: Emitter) -> str:
    ...
    for arg in fn.sig.args:
        args.append(
            "{}{}{}".format(emitter.ctype_spaced(arg.type), REG_PREFIX, arg.name)
        )
...
class FunctionEmitterVisitor(OpVisitor[None]):
    ...
    def visit_branch(self, op: Branch) -> None:
        ...
        if op.op == Branch.BOOL:
            ...
            if isinstance(typ, RTuple):
                ...
                cond = "{} {} {}".format(
                    self.reg(op.value), compare, self.c_error_value(typ)
                )
        else:
    ...
    def visit_load_error_value(self, op: LoadErrorValue) -> None:
        if isinstance(op.type, RTuple):
            ...
            tmp = self.temp_name()
            self.emit_line(
                "%s %s = { %s };" % (self.ctype(op.type), tmp, ", ".join(values))
            )
            self.emit_line("%s = %s;" % (self.reg(op), tmp))
        else:
            self.emit_line("%s = %s;" % (self.reg(op), self.c_error_value(op.type)))
    ...
    def visit_load_literal(self, op: LoadLiteral) -> None:
        ...
        if not is_int_rprimitive(op.type):
            ...
            self.emit_line(
                "%s = (CPyTagged)CPyStatics[%d] | 1;%s" % (self.reg(op), index, ann)
            )

    def get_attr_expr(
        self, obj: str, op: Union[GetAttr, SetAttr], decl_cl: ClassIR
    ) -> str:
    ...
    def visit_method_call(self, op: MethodCall) -> None:
        ...
        obj_args = (
            ...
            else ["(PyObject *)Py_TYPE({})".format(obj)]
    ...
    def visit_cast(self, op: Cast) -> None:
        self.emitter.emit_cast(
            self.reg(op.src), self.reg(op), op.type, src_type=op.src.type
        )
    ...
    def visit_comparison_op(self, op: ComparisonOp) -> None:
        ...
        self.emit_line(
            "%s = %s%s %s %s%s;"
            % (dest, lhs_cast, lhs, op.op_str[op.op], rhs_cast, rhs)
        )
