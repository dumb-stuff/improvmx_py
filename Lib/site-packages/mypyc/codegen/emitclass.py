...
def generate_class_type_decl(
    ...
    context.declarations[name] = HeaderDeclaration(
        "PyTypeObject *{};".format(emitter.type_struct_name(cl)), needs_export=True
    )
...
def generate_class(cl: ClassIR, module: str, emitter: Emitter) -> None:
    ...
    emitter.emit_line(
        "static PyTypeObject {}_template_ = {{".format(emitter.type_struct_name(cl))
    )
        ...
        if not base.is_trait:
            for attr, rtype in base.attributes.items():
                if (attr, rtype) not in seen_attrs:
                    lines.append(
                        "{}{};".format(emitter.ctype_spaced(rtype), emitter.attr(attr))
                    )
...
def generate_vtables(
    ...
    return (
        vtable_name
        if not subtables
        else "{} + {}".format(vtable_name, len(subtables) * 3)
    )
...
def generate_setup_for_class(
    ...
    emitter.emit_line(
        "self = ({struct} *)type->tp_alloc(type, 0);".format(
...
def generate_constructor_for_class(
    ...
    # If there is a nontrivial ctor that we didn't define, invoke it via tp_init
        ...
        emitter.emit_line(
            "int res = {}->tp_init({});".format(emitter.type_struct_name(cl), args)
        )
...
def generate_new_for_class(
    cl: ClassIR, func_name: str, vtable_name: str, setup_name: str, emitter: Emitter
) -> None:
    emitter.emit_line("static PyObject *")
    emitter.emit_line(
...
def generate_traverse_for_class(cl: ClassIR, func_name: str, emitter: Emitter) -> None:
    ...
    emitter.emit_line("static int")
    emitter.emit_line(
        "{}({} *self, visitproc visit, void *arg)".format(
...
def generate_dealloc_for_class(
    ...
    emitter.emit_line(
        "{}({} *self)".format(dealloc_func_name, cl.struct_name(emitter.names))
    )
    emitter.emit_line("{")
...
def generate_getseters_table(cl: ClassIR, name: str, emitter: Emitter) -> None:
    ...
    for prop in cl.properties:
        ...
        if setter:
            emitter.emit_line(
                " (setter){},".format(setter_name(cl, prop, emitter.names))
            )
            emitter.emit_line("NULL, NULL},")
        else:
...
def generate_getter(cl: ClassIR, attr: str, rtype: RType, emitter: Emitter) -> None:
    ...
    emitter.emit_line(
        '    "attribute {} of {} undefined");'.format(repr(attr), repr(cl.name))
    )
    emitter.emit_line("return NULL;")
    emitter.emit_line("}")
...
def generate_setter(cl: ClassIR, attr: str, rtype: RType, emitter: Emitter) -> None:
    ...
    if rtype.is_unboxed:
        emitter.emit_unbox(
            "value", "tmp", rtype, custom_failure="return -1;", declare_dest=True
        )
...
def generate_property_setter(
    ...
    if arg_type.is_unboxed:
        emitter.emit_unbox(
            "value", "tmp", arg_type, custom_failure="return -1;", declare_dest=True
        )
        emitter.emit_line(
            "{}{}((PyObject *) self, tmp);".format(
                NATIVE_PREFIX, func_ir.cname(emitter.names)
            )
        )
    else:
        emitter.emit_line(
            "{}{}((PyObject *) self, value);".format(
                NATIVE_PREFIX, func_ir.cname(emitter.names)
            )
        )
    emitter.emit_line("return 0;")
