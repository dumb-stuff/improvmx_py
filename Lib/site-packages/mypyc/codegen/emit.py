...
class Emitter:
    ...
    def static_name(
        self, id: str, module: Optional[str], prefix: str = STATIC_PREFIX
    ) -> str:
    ...
    def emit_unbox(
        ...
        if is_int_rprimitive(typ) or is_short_int_rprimitive(typ):
            ...
            self.emit_arg_check(
                src, dest, typ, "(likely(PyLong_Check({})))".format(src), optional
            )
            if borrow:
                self.emit_line(
                    "    {} = CPyTagged_BorrowFromObject({});".format(dest, src)
                )
            else:
            ...
            self.emit_arg_check(
                src, dest, typ, "(unlikely(!PyBool_Check({}))) {{".format(src), optional
            )
            self.emit_lines(*failure)
            self.emit_line("} else")
            ...
            if declare_dest:
                self.emit_line("char {};".format(dest))
            self.emit_arg_check(
                src, dest, typ, "(unlikely({} != Py_None)) {{".format(src), optional
            )
            self.emit_lines(*failure)
            self.emit_line("} else")
            ...
            self.emit_tuple_cast(
                src, cast_temp, typ, declare_dest=True, err="", src_type=None
            )
            ...
            for i, item_type in enumerate(typ.types):
                ...
                self.emit_line(
                    "PyObject *{} = PyTuple_GET_ITEM({}, {});".format(temp, src, i)
                )
    ...
    def emit_box(
        ...
        if is_int_rprimitive(typ) or is_short_int_rprimitive(typ):
            ...
            self.emit_line(
                "{}{} = CPyTagged_StealAsObject({});".format(declaration, dest, src)
            )
        elif is_bool_rprimitive(typ) or is_bit_rprimitive(typ):
            ...
            self.emit_lines(
                "{}{} = {} ? Py_True : Py_False;".format(declaration, dest, src)
            )
            if not can_borrow:
            ...
            self.emit_line(
                "{}{} = PyLong_FromLongLong({});".format(declaration, dest, src)
            )
        elif isinstance(typ, RTuple):
            self.declare_tuple_struct(typ)
            self.emit_line(
                "{}{} = PyTuple_New({});".format(declaration, dest, len(typ.types))
            )
            ...
            for i in range(0, len(typ.types)):
                if not typ.is_unboxed:
                    self.emit_line(
                        "PyTuple_SET_ITEM({}, {}, {}.f{}".format(dest, i, src, i)
                    )
                else:
                    ...
                    self.emit_line(
                        "PyTuple_SET_ITEM({}, {}, {});".format(dest, i, inner_name)
                    )
        else:
