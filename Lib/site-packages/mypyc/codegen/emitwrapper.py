...
def generate_wrapper_function(
    ...
    if groups[ARG_STAR] or groups[ARG_STAR2]:
        arg_ptrs += [
            "&obj_{}".format(groups[ARG_STAR][0].name) if groups[ARG_STAR] else "NULL"
        ]
        arg_ptrs += [
            "&obj_{}".format(groups[ARG_STAR2][0].name) if groups[ARG_STAR2] else "NULL"
        ]
...
def generate_get_wrapper(cl: ClassIR, fn: FuncIR, emitter: Emitter) -> str:
    ...
    emitter.emit_line(
        "return {}{}(self, instance, owner);".format(
...
def generate_del_item_wrapper(cl: ClassIR, fn: FuncIR, emitter: Emitter) -> str:
    ...
    input_args = ", ".join("PyObject *obj_{}".format(arg.name) for arg in fn.args)
    emitter.emit_line(
        "static int {name}({input_args}) {{".format(
            name=name,
            input_args=input_args,
        )
    )
...
def generate_set_del_item_wrapper(cl: ClassIR, fn: FuncIR, emitter: Emitter) -> str:
    ...
    if del_name is not None:
        ...
        emitter.emit_line(
            "PyObject *super = CPy_Super(CPyModule_builtins, obj_{});".format(
                args[0].name
            )
        )
    ...
    if method_cls and method_cls[1] == cl:
        ...
        if method_cls is None and cl.builtin_base is None:
            ...
            emitter.emit_line('PyErr_SetString(PyExc_TypeError, "{}");'.format(msg))
...
def generate_contains_wrapper(cl: ClassIR, fn: FuncIR, emitter: Emitter) -> str:
    ...
    name = "{}{}{}".format(DUNDER_PREFIX, fn.name, cl.name_prefix(emitter.names))
    emitter.emit_line(
        "static int {name}(PyObject *self, PyObject *obj_item) {{".format(name=name)
    )
...
def generate_wrapper_core(
    ...
    if fn.ret_type.is_unboxed or use_goto:
        ...
        emitter.emit_line(
            "return {};".format("retbox" if fn.ret_type.is_unboxed else "retval")
        )
    else:
        emitter.emit_line(
            "return {}{}({});".format(
                NATIVE_PREFIX, fn.cname(emitter.names), native_args
            )
        )
        # TODO: Tracebacks?
...
def generate_arg_check(
    name: str, typ: RType, emitter: Emitter, error_code: str, optional: bool = False
) -> None:
