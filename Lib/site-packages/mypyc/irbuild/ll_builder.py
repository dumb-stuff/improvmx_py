...
from typing import Callable, List, Tuple, Optional, Union, Sequence, cast
...
from mypyc.primitives.list_ops import list_extend_op, new_list_op
...
class LowLevelIRBuilder:
    ...
    def coerce(
        ...
        if src.type.is_unboxed and not target_type.is_unboxed:
            return self.box(src)
        if (src.type.is_unboxed and target_type.is_unboxed) and not is_runtime_subtype(
        ...
        if (not src.type.is_unboxed and target_type.is_unboxed) or not is_subtype(
            src.type, target_type
        ):
    ...
    def _vectorcall_keywords(
        self, arg_names: Optional[Sequence[Optional[str]]]
    ) -> Value:
    ...
    def native_args_to_positional(
        ...
        for lst, arg in zip(formal_to_actual, sig.args):
            ...
            if arg.kind == ARG_STAR:
                ...
                dict_entries = [
                    (self.load_str(cast(str, arg_names[i])), args[i]) for i in lst
                ]
    ...
    def gen_method_call(
        ...
        # Python method call.
        if arg_kinds is not None and not all(
            kind in (ARG_POS, ARG_NAMED) for kind in arg_kinds
        ):
    ...
    def unary_op(self, lreg: Value, expr_op: str, line: int) -> Value:
        if (
            is_bool_rprimitive(lreg.type) or is_bit_rprimitive(lreg.type)
        ) and expr_op == "not":
    ...
    def add_bool_branch(
        ...
        if is_runtime_subtype(value.type, int_rprimitive):
            ...
            if value_type is not None:
                ...
                return
            elif not is_bool_rprimitive(value.type) and not is_bit_rprimitive(
                value.type
            ):
    ...
    def translate_special_method_call(
        ...
        call_c_op = self.matching_call_c(
            call_c_ops_candidates, [base_reg] + args, line, result_type
        )
        return call_c_op
