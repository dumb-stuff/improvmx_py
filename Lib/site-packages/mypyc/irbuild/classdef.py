...
def transform_class_def(builder: IRBuilder, cdef: ClassDef) -> None:
    ...
    if ir.is_ext_class:
        ...
        non_ext_dict = setup_non_ext_dict(
            builder, cdef, non_ext_metaclass, non_ext_bases
        )
        ...
        if isinstance(stmt, OverloadedFuncDef) and stmt.is_property:
            ...
            if var_non_ext:
                add_non_ext_class_attr(
                    builder, var_non_ext, lvalue, stmt, cdef, attrs_to_cache
                )
                if non_ext:
                    continue
    ...
    if not non_ext:  # That is, an extension class
        ...
        builder.add(
            InitStatic(non_ext_class, cdef.name, builder.module_name, NAMESPACE_TYPE)
        )

        # Add the non-extension class to the dict
        builder.call_c(
            dict_set_item_op,
            [builder.load_globals_dict(), builder.load_str(cdef.name), non_ext_class],
            cdef.line,
        )
...
def setup_non_ext_dict(
    ...
    true_block, false_block, exit_block, = (
        BasicBlock(),
        BasicBlock(),
        BasicBlock(),
    )
...
def finish_non_ext_dict(
    builder: IRBuilder, non_ext: NonExtClassInfo, line: int
) -> None:
    # Add __annotations__ to the class dict.
    builder.call_c(
        dict_set_item_op,
        [non_ext.dict, builder.load_str("__annotations__"), non_ext.anns],
        -1,
    )
...
def dataclass_finalize(
    builder: IRBuilder, cdef: ClassDef, non_ext: NonExtClassInfo, type_obj: Value
) -> None:
    ...
    builder.call_c(
        dataclass_sleight_of_hand,
        [dec, type_obj, non_ext.dict, non_ext.anns],
        cdef.line,
    )


def dataclass_non_ext_info(
    builder: IRBuilder, cdef: ClassDef
) -> Optional[NonExtClassInfo]:
