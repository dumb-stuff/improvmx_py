...
if IS_32_BIT_PLATFORM:
    ...
    c_pyssize_t_rprimitive = RPrimitive(
        "native_int", is_unboxed=True, is_refcounted=False, ctype="int32_t", size=4
    )
else:
    c_size_t_rprimitive = uint64_rprimitive
    c_pyssize_t_rprimitive = RPrimitive(
        "native_int", is_unboxed=True, is_refcounted=False, ctype="int64_t", size=8
    )

# Low level pointer, represented as integer in C backends
pointer_rprimitive = RPrimitive(
...
float_rprimitive = RPrimitive(
    "builtins.float", is_unboxed=False, is_refcounted=True
)  # type: Final
...
dict_next_rtuple_single = RTuple([bool_rprimitive, int_rprimitive, object_rprimitive])
