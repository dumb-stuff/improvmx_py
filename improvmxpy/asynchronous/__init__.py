...
class Account(object):
    ...
    async def GetAccountDetail(self):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{url}account", auth=aiohttp.BasicAuth("api", self.token)
            ):
    ...
    async def GetWhiteLabelDomain(self):
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{url}account/whitelabels", auth=aiohttp.BasicAuth("api", self.token)
            ):
                return await self.__CheckResponse(r)
...
class Domains(object):
    ...
    async def DomainDetail(self, domain):
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{url}domains/{domain}", auth=aiohttp.BasicAuth("api", self.token)
            ):
                return await self.__CheckResponse(r)
...
class Logging(object):
    ...
    async def GetDomainLog(self, domain, logID=None):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{url}domains/{domain}/logs", auth=aiohttp.BasicAuth("api", self.token)
            ):
                return await self.__CheckResponse(r)
